...
export function recursiveScan(
  ns: NS,
  startServer: string,
  maximumPortRequirement = -1,
  includeServersWithoutRam = true,
  includePlayerOwnedServers = true,
): string[] {
  const servers = _recursiveScan(ns, startServer);
  return servers.filter((e) => {
    return (
      ((maximumPortRequirement < 0 ||
        ns.getServerNumPortsRequired(e) <= maximumPortRequirement) &&
...
function _recursiveScan(ns: NS, server: string, previousServer = ""): string[] {
  const result = [server];
  const servers = ns.scan(server).filter((e) => {
...
function printScannedServers(
  ns: NS,
  currentServer: string,
  prefix = "",
  previousServer = "",
): void {
  const servers = ns.scan(currentServer).filter((e) => {
    return e !== previousServer;
  });
